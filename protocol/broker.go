	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				log.Info("serf members:  %v", b.Serf.Members())
				future := b.Raft.GetConfiguration()
				if err := future.Error(); err != nil {
					log.Error("broker/%d: get raft configuration error: %s", b.Config.NodeID, err)

				} else {
					log.Info("future.Configuration().Servers : %+v ", future.Configuration().Servers)
					log.Info("Node : %v %v ", b.Raft.State(), b.Raft)
				}

				log.Info("LeaderAddr: [%v] - leaderID [%v]", leaderAddr, leaderID) // Execute this code after 5 seconds

				nodes, err := b.GetClusterNodes()
				log.Info("b.GetClusterNodes(): %v  error :%v", nodes, err)

				if b.Raft.State() == hraft.Leader {
					future := b.Raft.Apply([]byte(`{"Kind": 0, "Payload":{"Key": "DDDD`+strconv.Itoa(i)+`", "Value":"XXX"}}`), 500*time.Millisecond)
					if err := future.Error(); err != nil {
						log.Error("Could not write key-value: %s", err)
					}
					i++
				} else {
					log.Info("Node is follower. leaderAddr : %v", string(leaderAddr))
				}

				log.Info("len(b.FSM.Test) %+v", len(b.FSM.Test))
			}
		}
	}()
